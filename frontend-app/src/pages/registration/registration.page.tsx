...
import { useRegistrationPage } from "./registration.hook";

const RegistrationPage = memo(() => {
  const hook = useRegistrationPage();
  ...
  return (
    <div className="flex flex-col gap-2 min-w-96 max-w-96">
      ...
      <form
        ...
        onSubmit={hook.handleSubmit(hook.handleRegistration)}
      >
        ...
        <div className="mb-6">
          ...
          <div className="relative">
            ...
            <button
              type="button"
              className="absolute right-3 top-2.5 text-sm text-gray-600 hover:text-gray-800 focus:outline-none cursor-pointer"
              onClick={hook.toggleConfirmPasswordVisibility}
            >
              {hook.showConfirmPassword ? "Скрыть" : "Показать"}
            </button>
          </div>
          {hook.errors.confirmPassword && (
            <p className="text-red-500 text-sm mt-1">{hook.errors.confirmPassword.message}</p>
          )}
        </div>
        ...
        >
          {hook.isPending ? "Загрузка..." : "Зарегистрироваться"}
        </button>
      </form>

      <button
        onClick={() => hook.setAuthStep("authorization")}
        className="text-sm text-blue-500 hover:underline focus:outline-none cursor-pointer"
      >
        <span className="text-black">Уже зарегистрированы?</span> Войти!
      </button>
    </div>
  );
});
